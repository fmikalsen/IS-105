...
#     Data leses med recv(), så blir plassert i en kø, slik at den kan bli sendt gjennom socketen tilbake til klienten
			...
			if data:
				...
				if data == "join\n":
					if numberOfPlayers >= allowedPlayers:
						...
						joined = "Det er %d av %d spiller(e) med" % (numberOfPlayers, allowedPlayers)
				...
				if data == "geth\n":
					if dealt == True:
						...
						dealFirst = "Du må dele ut kort først"
						message_queues[s].put(dealFirst)
						
				if data == "players\n":
					spiller = str(board)
					message_queues[s].put(spiller)
